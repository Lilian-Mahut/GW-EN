...
const niCycle = require("./config/niCycle.json")
...
let thisDay = currentDate.getDay() + 1
...
const laDate = daysOfWeek[thisDay] + " " + thisDate + " " + monthOfYear[thisMonth] + " " + thisYear
console.log(laDate)
...
function reset() {
  ...
  else thisWeek = thisDay // return false ?
  // console.log(thisDay) // return true ?
  // console.log(postDay) // 2
  // console.log(thisWeek) // 2
}
...
bot.on('message', message => {
  ...
  if (message.content === cfg.prefix + '!') { 
    // avec le mois, date et heure => chercher l'info dans event.json
    // message.author.send(event[mois].[jour].name)
  }
  // ZAISHEN
  if (message.content === cfg.prefix + 'z') { message.author.send("work in progress...") }
  if (message.content === cfg.prefix + 'cz') { message.author.send("work in progress...") }
  if (message.content === cfg.prefix + 'mz') { message.author.send("work in progress...") }
  if (message.content === cfg.prefix + 'pz') { message.author.send("work in progress...") }
  if (message.content === cfg.prefix + 'zv') { message.author.send("work in progress...") }
  // EDEN
  if (message.content === cfg.prefix + 'e') { message.author.send("work in progress...") }
    // {
    // embed: {
    //   color: color.green,
    //   title: "eden",
  ...
  if (message.content === cfg.prefix + 'j') { message.author.send({
    embed: {
      ...
      fields: [
        ...
        { name: eden.nameNic + " " + eden.cmd, value: "edenNico" },
